...
const bool& allowDiagonal, const bool& allowCrossCorners, const int& searchType) : grid(grid) {
	...
	this->searchType = searchType > 2 ? 0 : searchType;
...
void PathFinder::getNextPath(const int& x, const int& y) {
	...
	//up down
	if (possiblePassthrough(x, y - 1)) { toCheck.emplace_back(x, y - 1); } else { updateCrossCorner(NORTH_WEST); updateCrossCorner(NORTH_EAST); }
	if (possiblePassthrough(x, y + 1)) { toCheck.emplace_back(x, y + 1); } else { updateCrossCorner(SOUTH_WEST); updateCrossCorner(SOUTH_EAST); }

	//left right
	if (possiblePassthrough(x + 1, y)) { toCheck.emplace_back(x + 1, y); } else { updateCrossCorner(NORTH_EAST); updateCrossCorner(SOUTH_EAST); }
	if (possiblePassthrough(x - 1, y)) { toCheck.emplace_back(x - 1, y); } else { updateCrossCorner(NORTH_WEST); updateCrossCorner(SOUTH_WEST); }
	...
	if (possiblePassthrough(x + 1, y + 1) && isCornerPossible[SOUTH_EAST]) { toCheck.emplace_back(x + 1, y + 1); }
...
void PathFinder::updateNeighbourNode(const int& x, const int& y) {
	...
	for (int i = 0; i < searchList.size(); ++i) {
		...
		if (searchType == SEARCH_TYPE_A_STAR) { grid[searchList[i].first][searchList[i].second].set_hCost(hCost); }
...
void PathFinder::setCost() {
	...
	for (int i = 0, parentCounter = 0; i < toCheck.size(); ++i) {
		...
		if (searchType == SEARCH_TYPE_A_STAR) {
